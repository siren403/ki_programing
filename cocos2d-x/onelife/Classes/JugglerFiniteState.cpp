...
void JugglerIdleState::OnUpdate(float dt)
{
	if (mIsActive)
	{
		...
		Vec2 pos;
		pos = ccpLerp(this->GetEntity()->getPosition(), player->getPosition(), dt * 0.35f);
		this->GetEntity()->setPosition(pos);

		mStopWatch->OnUpdate(dt);

		if (mStopWatch->GetAccTime() > 2.5f)
		{
			GetEntity<Juggler>()->RandomChangeState();
...
void JugglerSeqAttackState::OnUpdate(float dt)
{
	...
	if (mCurrentAttackIndex <  circleCount)
	{
		if (mStopWatch->GetAccTime() >= 1.2f)
		{
			...
			circle->OnAttack(targetPos, 2.7f);
			mCurrentAttackIndex++;

			circle = (JugglerCircle*)this->GetEntity<Juggler>()->GetParts(mCurrentAttackIndex);
			targetPos = ActorManager::GetInstance()->ConvertPlayerToEntity(mEntity);
			circle->OnAttack(targetPos, 3.7f);
			mCurrentAttackIndex++;
...
#pragma region warning
	...
	for (int y = 0; y < map->GetMapHeight(); y++)
	{
		for (int x = 0; x < map->GetMapWidth(); x++)
		{
			...
			if (radius < rushRange)
			{
				map->GetTile(curTileIndex)->SetHighlight(true, mChargeDuration * 0.7f + radius * 0.05f);
			}
		}
	}
...
void JugglerRushAttackState::OnUpdate(float dt)
{
	...
	else//rush
	{
		...
		if (mStopWatch->GetAccTime() >= mRushDuration / 2)
		{
			...
			mStopWatch->OnReset();
			GetEntity<Juggler>()->RandomChangeState();
			//mEntity->ChangeState(random(0.0f, 1.0f) > 0.5f ? Juggler::State::State_Idle : Juggler::State::State_CornerAttack);
		}
	}
}
...
void JugglerCornerAttackState::InitMoveEasingData(EasingType type, Vec2 start, Vec2 end)
{
	...
	mMoveXEasingData.duration = 1.4f;
	...
	mMoveYEasingData.duration = 1.4f;
}
...
void JugglerColRowAttack::OnEnter()
{
	...
	mState = random(0.0f, 1.0f) > 0.5f ? State::State_HorizonPosition : State::State_VerticalPosition;
...
void JugglerColRowAttack::UpdateVerticalPosition(float dt)
{
	...
	for (int i = 0; i < mCircleCount; i++)
	{
		...
		data.changeValue = (mMapSize.height * (i * 0.07f)) + mMapSize.height * 0.1f - data.startValue;
	...
	data.changeValue = (mMapSize.width * 0.1f) - data.startValue;
	pos.x = EasingFunc::Ease(data);
...
void JugglerColRowAttack::UpdateVerticalMove(float dt)
{
	...
	Vec2 pos;
	data.startValue = mMapSize.width * 0.1f;
	data.changeValue = (mMapSize.width * 0.9f) - data.startValue;
	pos.x = EasingFunc::Ease(data);
	pos.y = mEntity->getPosition().y;
	mEntity->setPosition(pos);
...
void JugglerColRowAttack::UpdateHorizonPosition(float dt)
{
	...
	{
		data.startValue = mInitPositions[i].x;
		data.changeValue = (mMapSize.width * (i * 0.06f)) + mMapSize.width * 0.13f - data.startValue;
		pos.x = EasingFunc::Ease(data);

		data.startValue = mInitPositions[i].y;
		data.changeValue = (mMapSize.height * 0.9f) - data.startValue;
		pos.y = EasingFunc::Ease(data);
...
void JugglerColRowAttack::UpdateHorizonMove(float dt)
{
	...
	Vec2 pos;
	data.startValue = mMapSize.height * 0.9f;
	data.changeValue = (mMapSize.height * 0.1f) - data.startValue;
	pos.x = mEntity->getPosition().x;
	pos.y = EasingFunc::Ease(data);
	mEntity->setPosition(pos);
